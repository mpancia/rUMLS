...
{
  exhaust_search(FUN = get_source_info_page, source = source, id = id)
}

#' @rdname get_source_info
get_source_info_page <- function(source, id, pageNumber = 1, pageSize = 25 ){
  params = list(ticket = get_service_ticket(get_TGT()), pageNumber = pageNumber, pageSize = pageSize)
  r <- GET(restBaseURL, path = paste0("rest/content/current/", source, "/", id), query = params)
  r
}
...
#' @export
#'
get_source_atoms <- function(source, id, sabs = NULL, ttys = NULL, language = NULL, includeObsolete = FALSE, includeSuppressible = FALSE,
                              preferred = FALSE)
{
  exhaust_search(FUN = get_source_atoms_page, source = source, id = id, sabs = sabs, ttys = ttys, language = language,
                  includeObsolete = includeObsolete, includeSuppressible = includeSuppressible, preferred = preferred)
}

#' @rdname get_source_atoms
get_source_atoms_page <- function(source, id, sabs = NULL, ttys = NULL, language = NULL, includeObsolete = FALSE,
                                  includeSuppressible = FALSE, pageNumber = 1, pageSize = 25, preferred = FALSE )
{
  params = list(ticket = get_service_ticket(get_TGT()), sabs = sabs, ttys = ttys, language = language,
                includeObsolete = includeObsolete, includeSuppressible = includeSuppressible, pageNumber = pageNumber, pageSize = pageSize)
  if(preferred == TRUE)
  {
